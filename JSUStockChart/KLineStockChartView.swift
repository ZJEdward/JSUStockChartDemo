...
open class KLineStockChartView:KLineStockChartViewBase {
    ...
    var avgLabelAttributedDic : [String:AnyObject] = [NSFontAttributeName:UIFont.systemFont(ofSize: 8),NSBackgroundColorAttributeName:UIColor.clear,NSForegroundColorAttributeName:UIColor(netHex: 0x8695a6, alpha: 1)]
            ...
            for i in idex ..< data.count {
                ...
                if let date = entity.date.toDate("yyyy-MM-dd"){
                    ...
                    if date.year > oldDate?.year || date.month > oldDate!.month + monthLineLimit {
                        self.drawline(context, startPoint: CGPoint(x: startX, y: self.contentTop), stopPoint: CGPoint(x: startX,  y: (self.uperChartHeightScale * self.contentHeight) + self.contentTop), color: self.borderColor, lineWidth: 0.5)
                        self.drawline(context, startPoint: CGPoint(x: startX, y: (self.uperChartHeightScale * self.contentHeight) + self.xAxisHeitht), stopPoint: CGPoint(x: startX,y: self.contentBottom), color: self.borderColor, lineWidth: 0.5)
                        ...
                        if !self.highlightLineCurrentEnabled{
                            ...
                            self.drawLabel(context, attributesText: dateStrAtt, rect: CGRect(x: startX - dateStrAttSize.width/2,y: ((self.uperChartHeightScale * self.contentHeight) + self.contentTop), width: dateStrAttSize.width, height: dateStrAttSize.height))
                        }
