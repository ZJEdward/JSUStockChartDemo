...
class JSUTimeViewController: UIViewController {
    ...
    func setupTradeView(_ data:JSUTradeModel){
        if let bp = data.bp1{
            self.buy1Price.text = bp.toDouble().toStringWithFormat("%.2f")
            if let o = self.openPrice, bp.toDouble() >= o{
        ...
        if let sp = data.sp1{
            ...
            if let o = self.openPrice, sp.toDouble() >= o{
    ...
    func setupTimeLineView(_ data:JSUPriceModel){
    ...
    override func viewDidLoad() {
        ...
        self.timeView.gridBackgroundColor = UIColor.white
